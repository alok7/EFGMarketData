...
protected:
  struct Socket {
    socket_t sock = INVALID_SOC;
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    SSL *ssl = nullptr;
#endif

    bool is_open() const { return sock != INVALID_SOC; }
  };
...
inline Server::Server()
    : new_task_queue(
      ...
      svr_sock_(INVALID_SOC), is_running_(false) {
#ifndef _WIN32
...
inline void Server::stop() {
  if (is_running_) {
    assert(svr_sock_ != INVALID_SOC);
    std::atomic<socket_t> sock(svr_sock_.exchange(INVALID_SOC));
...
Server::write_content_with_provider(Stream &strm, const Request &req,
  ...
  auto is_shutting_down = [this]() {
    return this->svr_sock_ == INVALID_SOC;
  };
...
inline int Server::bind_internal(const char *host, int port, int socket_flags) {
  ...
  if (svr_sock_ == INVALID_SOC) { return -1; }
...
inline bool ClientImpl::create_and_connect_socket(Socket &socket,
  ...
  if (sock == INVALID_SOC) { return false; }
  socket.sock = sock;
  return true;
}
...
inline void ClientImpl::shutdown_socket(Socket &socket) {
  if (socket.sock == INVALID_SOC) { return; }
