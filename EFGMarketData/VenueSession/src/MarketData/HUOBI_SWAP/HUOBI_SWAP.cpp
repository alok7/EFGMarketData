...
namespace MarketData
{
  ...
  {
    HUOBI_SWAP::HUOBI_SWAP(const ConfigManager::VenueSessionInfo &vsinfo) : AbstractVenue(vsinfo)
    {
      transport = std::make_unique<Transport::WssTransport>(&md_hub);
      registerCallBacks();
      MD_LOG_INFO << "Created  HUOBI_SWAP Session " << '\n';
    }
    void HUOBI_SWAP::onConnection(uWS::WebSocket<uWS::CLIENT> *ws, uWS::HttpRequest req)
    {
      MD_LOG_INFO << " HUOBI_SWAP Connected " << '\n';
      ...
      auto timerCallBack = [this]()
      {
        ...
        {
          MD_LOG_INFO << "HUOBI_SWAP No updates in last 6 sec " << '\n';
    ...
    void HUOBI_SWAP::reConnect()
    {
      MD_LOG_INFO << "HUOBI_SWAP Reconnecting..." << '\n';
    ...
    void HUOBI_SWAP::onUpdate(uWS::WebSocket<uWS::CLIENT> *ws, char *message, std::size_t len_, uWS::OpCode code)
    {
      try
      {
    ...
    void HUOBI_SWAP::onDisconnection(uWS::WebSocket<uWS::CLIENT> *ws, int code, char *message, std::size_t length)
    {
      try
      {
        _isConnected.store(false);
        MD_LOG_INFO << " HUOBI_SWAP Disconnected " << std::to_string(Utils::getHRTime()) << '\n';
        reConnect();
      }
      catch (const std::exception &e)
      {
        MD_LOG_INFO << " HUOBI_SWAP Exception on Disconnect Event " << e.what() << '\n';
      }
      catch (...)
      {
        MD_LOG_INFO << "HUOBI_SWAP Unknown Exception on Disconnet " << '\n';
      }
    }
    void HUOBI_SWAP::registerCallBacks()
    {
      using namespace std::placeholders;
      transport->registerOnConnectCallBack(std::bind(&HUOBI_SWAP::onConnection, this, _1, _2));
      transport->registerOnUpdateCallBack(std::bind(&HUOBI_SWAP::onUpdate, this, _1, _2, _3, _4));
      transport->registerOnDisconnectCallback(std::bind(&HUOBI_SWAP::onDisconnection, this, _1, _2, _3, _4));
    }
    void HUOBI_SWAP::decode(char *buffer, size_t len, uWS::WebSocket<uWS::CLIENT> *ws, const TimeType &packet_hit_time)
    {
      Document decoder;
      ...
      case Utils::hash_32_fnv1a("ch"):
      {
        ...
        case Utils::hash_32_fnv1a("depth.step11"):
        {
          ...
          normalized_depth.venue = MarketUtils::MarketType::HUOBI_SWAP;
        ...
        case Utils::hash_32_fnv1a("trade.detail"):
        {
          ...
          // MD_LOG_INFO << "HUOBI Trade: " << buffer << '\n';
          ...
          for (int i = data.Size()-1; i >= 0; i--)
          {
            ...
            normalized_msg_type normalized_trade(data[i]["price"].GetDouble() * 1000000000,
                                                  ...
                                                  side);
            normalized_trade.venue = MarketUtils::MarketType::HUOBI_SWAP;
          ...
          // {
          //   sendTradeUpdate(MarketUtils::MarketType::HUOBI_SWAP, symbol);
          // }
          break;
        }
    ...
    void HUOBI_SWAP::decodeBinary(char *buffer, size_t len)
    {
    }
  }
}
