#include <MarketData/bstree_algorithms_base.h>
#include <math.h>

#ifndef BSTREE_ALGORITHMS_H
#define BSTREE_ALGORITHMS_H

template <class NodePtr>
struct insert_commit_data_t
{
   FORCEINLINE insert_commit_data_t(): link_left(false), node() {}
   bool     link_left;
   NodePtr  node;
};
...
class bstree_algorithms : public bstree_algorithms_base<NodeTraits>
{
	...
	{
		FORCEINLINE dispose_subtree_disposer(Disposer &disp, const node_ptr & subtree)
		...
		FORCEINLINE ~dispose_subtree_disposer()
		{
			if(disposer_){
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static node_ptr begin_node(const const_node_ptr & header)
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static node_ptr end_node(const const_node_ptr & header)
	{  return const_cast<node_ptr&>(header);   }
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static node_ptr root_node(const const_node_ptr & header)
	{
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static bool unique(const const_node_ptr & node)
	...
	static void swap_nodes(node_ptr node1, node_ptr header1, node_ptr node2, node_ptr header2)
	{
		...
		//ASSERT((header1 != node1 && header2 != node2));
		if(header1 != header2){
	...
	FORCEINLINE static void replace_node(node_ptr node_to_be_replaced, node_ptr new_node)
	{
	...
	FORCEINLINE static void init(node_ptr node)
	{
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static bool inited(const const_node_ptr & node)
	{
		return !NodeTraits::get_parent(node) &&
	...
	//!
	//! <b>Nodes</b>: If node is inserted in a tree, this function corrupts the tree.
	FORCEINLINE static void init_header(node_ptr header)
	{
	...
	//! <b>Throws</b>: If "comp" throws.
	template<class KeyType, class KeyNodePtrCompare>
	FORCEINLINE static std::pair<node_ptr, node_ptr> equal_range
		(const const_node_ptr & header, const KeyType &key, KeyNodePtrCompare comp)
	{
	...
	//! <b>Throws</b>: If "comp" throws.
	template<class KeyType, class KeyNodePtrCompare>
	FORCEINLINE static node_ptr lower_bound
		(const node_ptr & header, const KeyType &key, KeyNodePtrCompare comp)
	{
		return lower_bound_loop(NodeTraits::get_parent(header), header, key, comp);
	}
	...
	//! <b>Throws</b>: If "comp" throws.
	template<class KeyType, class KeyNodePtrCompare>
	FORCEINLINE static node_ptr upper_bound
		(const const_node_ptr & header, const KeyType &key, KeyNodePtrCompare comp)
	{
		return upper_bound_loop(NodeTraits::get_parent(header), header, key, comp);
	}
	...
	FORCEINLINE static void insert_unique_commit
	...
	{
		std::size_t depth = 0;
		node_ptr h(const_cast<node_ptr&>(header));
		node_ptr y(h);
	...
	static std::pair<node_ptr, bool> insert_unique_check
		...
		//!
		//! <b>Throws</b>: Nothing.
		FORCEINLINE static void erase(node_ptr header, node_ptr z)
		{
		...
		template<class NodePtrCompare>
		FORCEINLINE static bool transfer_unique
		...
		//! <b>Throws</b>: If the comparison throws.
		template<class NodePtrCompare>
		FORCEINLINE static void transfer_equal
			(node_ptr header1, NodePtrCompare comp, node_ptr header2, node_ptr z)
		{
			data_for_rebalance ignored;
		...
		//! <b>Requires</b>: header is the header of the tree.
		//!
		//! <b>Effects</b>: Clears and destroys all nodes of the tree.
		//!
		//! <b>Complexity</b>: Linear in number of nodes.
		//!
		//! <b>Throws</b>: Nothing.
		static void clear(const node_ptr header) {
			clear_subtree(NodeTraits::get_parent(header));
			init_header(header);
		}

		//! <b>Requires</b>: pointer to the root of the subtree to be cleared
		//!
		//! <b>Effects</b>: Clears and destroys all nodes in the subtree rooted
		//!		at node including node itself.
		//!
		//! <b>Complexity</b>: Linear in number of nodes in the subtree.
		//!
		//! <b>Throws</b>: Nothing.
		static void clear_subtree(node_ptr node) {
			while (node) 
			{
				// MD_LOG_INFO << node->_val.first << ' ' << node->_val.second << "\n";
				clear_subtree(NodeTraits::get_right(node));
				node_ptr y = NodeTraits::get_left(node);
				// clear and destroy node node
				if (node) delete node;
				node = y;
			}
		}
		...
		static void check(const const_node_ptr& header, Checker checker, typename Checker::return_type& checker_return)
		{
			...
			if (!root_node_ptr){
				...
				ASSERT(NodeTraits::get_left(header) == header);
				ASSERT(NodeTraits::get_right(header) == header);
			}
			else{
				// check parent pointer of root node
				ASSERT(NodeTraits::get_parent(root_node_ptr) == header);
				...
				ASSERT(NodeTraits::get_left(header) == p);
				p = root_node_ptr;
				while (NodeTraits::get_right(p)) { p = NodeTraits::get_right(p); }
				ASSERT(NodeTraits::get_right(header) == p);
			}
		}
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static bool is_left_child(const node_ptr & p)
	...
	//!
	//! <b>Throws</b>: Nothing.
	FORCEINLINE static bool is_right_child(const node_ptr & p)
	...
	static void vine_to_subtree(node_ptr super_root, std::size_t count)
	{
		...
		std::size_t leaf_nodes = count + one_szt - std::size_t(one_szt << size_t(floor(log2(count + one_szt))));
	...
	static void check_subtree(const const_node_ptr& node, Checker checker, typename Checker::return_type& check_return)
	{
		...
		if (left)
		{
			ASSERT(NodeTraits::get_parent(left) == node);
			check_subtree(left, checker, check_return_left);
		}
		if (right)
		{
			ASSERT(NodeTraits::get_parent(right) == node);
